...
```bash
cd mmo-cr-push-notification-service
nvm use
```
...
```bash
docker build --no-cache --tag mmo-cr-push-notification-service .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 mmo-cr-push-notification-service
```
